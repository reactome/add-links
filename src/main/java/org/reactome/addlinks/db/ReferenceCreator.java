...
public class ReferenceCreator
{
	...
	{
		//TODO: Need to rethink this: Attempt to create a cross reference. If it already exists, print a warning and move on.
		// Normally, the references generated by AddLinks should NOT be in the database, but because they never get
		// back-propagated from reactomerelease to reactomecurator, they need to be re-created in every Release. 
		// But since I don't have a database copy that contains orthoinferenced entities but is missing the generated references
		// (and it's very complicated to figure out how to massage a test_reactome_xx database to look the way it should),
		// I'll just code it to ignore pre-existing references. In theory, I could add code to delete pre-existing references.
		// Also need to add an argument to help determine if the thing being created is a DatabaseIdentifier, ReferenceGeneProduction, ReferenceMolecule....
