...
public class XenbaseReferenceCreator extends SimpleReferenceCreator<List<String>>{
    public XenbaseReferenceCreator(MySQLAdaptor adapter, String classToCreate, String classReferring, String referringAttribute, String sourceDB, String targetDB)
    {
        super(adapter, classToCreate, classReferring, referringAttribute, sourceDB, targetDB);
    }

    public XenbaseReferenceCreator(MySQLAdaptor adapter, String classToCreate, String classReferring, String referringAttribute, String sourceDB, String targetDB, String refCreatorName)
    {
    ...
    public void createIdentifiers(long personID, Map<String, List<String>> mapping, List<GKInstance> sourceReferences) throws Exception
    {
        ...
        for (GKInstance sourceInst : sourceReferences)
        {
            ...
            {
                for (String xenbaseID : mapping.get(sourceIdentifier))
                {
                    if (!this.checkXRefExists(sourceInst, xenbaseID))
                    {
                        sourceIdentifiersWithNewIdentifier ++;
                        if (!this.testMode)
                        {
                            this.refCreator.createIdentifier(xenbaseID, String.valueOf(sourceInst.getDBID()), targetRefDB, personID, this.getClass().getName(), species);
                        }
                    }
                    else
                    {
